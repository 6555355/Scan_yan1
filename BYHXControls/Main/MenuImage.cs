...
namespace BYHXPrinterManager
{
	...
	public class MenuImage : Component, IExtenderProvider
	{
		...
		/// </summary>
		const int IMAGE_BUFFER = 25 *2;
		const int SHORTCUT_RIGHTBUFFER = 10*2 ;
		const int SHORTCUT_BUFFER = 25 *2;
		int IMAGE_WIDTH = SystemInformation.SmallIconSize.Width *2;
		int IMAGE_HEIGHT = SystemInformation.SmallIconSize.Height*2 ;
		...
		/// <param name="indexValue">the image index value to associate with the menu item</param>
		/// <exception cref="System.InvalidArgument">Image index is not a valid value.</exception>
		public void RemoveMenuImage( Component component)
		{
...
//				_hashTable.Remove(component);
		...
		public bool CanExtend( object component )
		{
			...
			{
				MenuItem menuItem = (MenuItem) component;
				return true;//! (menuItem.Parent is MainMenu ) ;
			}
        ...
        /// <summary>
        /// Gets or Sets the MenuItem's Font.
        /// </summary>
        public Font MenuFont
        {
            get { return MenuHelper.MenuFont; }
            set { MenuHelper.MenuFont = value; }
        }
        ...
        public class MenuHelper
		{
			...
			public static Font MenuFont = new Font(SystemInformation.MenuFont.FontFamily,SystemInformation.MenuFont.Size);
			private Size SmallIconSize = new Size(SystemInformation.SmallIconSize.Width,SystemInformation.SmallIconSize.Height);
			...
			/// <summary>
			/// Based on the menu item text, and the <c>SmallIconSize,</c>
			...
			public int CalcHeight()
			{
				...
				else
				{
					...
					if ( MenuFont.Height > SmallIconSize.Height )
						return MenuFont.Height + BORDER_SIZE ;
					else
						return SmallIconSize.Height + BORDER_SIZE ;
				}
			}
			...
			public int CalcWidth()
			{
				...
				int menuWidth = (int) _graphics.MeasureString( _menuItem.Text, MenuFont, 1000, sf).Width ;
				int shortcutWidth = (int) _graphics.MeasureString( this.ShortcutText, MenuFont, 1000, sf).Width ;
			...
			public void DrawMenu ( Rectangle bounds, bool selected, int indexValue )
			{
				...
				if ( _menuItem.IsParent == true && !(_menuItem.Parent is MainMenu))
				{
					Image menuImage = null ;
					System.IO.Stream stream = this.GetType().Assembly.GetManifestResourceStream("BYHXPrinterManager.Main.SubItem16.ico");//Chris.Beckett.MenuImageLib.SubItem16.ico") ;
			...
			private void DrawMenuText ( Rectangle bounds, bool selected )
			{
				...
				if(_menuItem.Parent is MainMenu)
					_graphics.DrawString( _menuItem.Text, menuFont, menuBrush, bounds.Left, bounds.Top + ((bounds.Height - menuFont.Height) / 2), sfMenu ) ;
				else
			...
			private void DrawImage( Image menuImage, Rectangle bounds )
			{
				...
				{
					Image thimImage = menuImage.GetThumbnailImage(IMAGE_WIDTH,IMAGE_HEIGHT,null,IntPtr.Zero);
					ControlPaint.DrawImageDisabled(_graphics, thimImage,
			...
			private void DrawArrow( Image menuImage, Rectangle bounds )
			{
				...
				{
					ControlPaint.DrawImageDisabled(_graphics, menuImage, 
						bounds.Left + bounds.Width - ARROW_WIDTH, bounds.Top + ((bounds.Height - IMAGE_HEIGHT) / 2),Color.White ) ;
				}
			}
