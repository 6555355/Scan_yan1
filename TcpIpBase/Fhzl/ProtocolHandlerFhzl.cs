namespace TcpIpBase {
    ...
    public class ProtocolHandlerFhzl:ProtocolHandler {
        internal string partialProtocal;	// 保存不完整的协议
        private string pattern;// 正则表达式匹配规则
        public ProtocolHandlerFhzl(string strPattern) : base(strPattern)
        {
			partialProtocal = "";
            pattern = strPattern;
            Helper = new ProtocolHelperFhzl();
        }

        public override List<Protocol> GetProtocol(string input)
        {
