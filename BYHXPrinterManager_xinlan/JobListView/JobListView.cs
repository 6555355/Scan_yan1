...
namespace BYHXPrinterManager.JobListView
{
	...
	public class JobListForm : System.Windows.Forms.UserControl
	{
		...
		public PrintingInfo m_PreviewAndInfo;
        ...
        public JobListForm()
		{
            ...
            m_PrintJobTask.JobCopyPrinted += new EventHandler(m_PrintJobTask_JobCopyPrinted);
        ...
        void m_PrintJobTask_JobCopyPrinted(object sender, EventArgs e)
        {
            if (UIFunctionOnOff.SupportGlogalAlternatingPrint
                && _printerProperty.IsAllWinUV()
            )
            {
                MessageBox.Show(ResString.GetResString("StrMakesureMediaReady"), ResString.GetProductName(), MessageBoxButtons.OK, MessageBoxIcon.Question);
            }
        }
        ...
        /// 数据处理现场是否在运行中
        /// </summary>
        public bool PrintJobTaskIsWorking
        {
            get
            {
                return m_PrintJobTask.IsWorking();
            }
        }
	    ...
	    public void OnPrinterStatusChanged(JetStatusEnum status,bool IsFATAL)
		{
            m_IsFATAL = IsFATAL;
		...
		public void OnPrinterSettingChange( SPrinterSetting ss)
		{
            ...
            if (m_MeasureQuestionForm != null)
            {
                m_MeasureQuestionForm.OnPrinterSettingChange(ss);
            }
		}
        ...
        public void OnJobStatusChanged(UIJob job)
		{
            ...
            if(jobStatus == JobStatus.Printed)
			{
				if(this.m_Preference.DelJobAfterPrint)
				{
                    ...
                    ListViewHelper.SetItemBackColor(this.m_JobListView);
                    if (SPrinterProperty.IsBIHONG_FLAT_TEXTILE())
                    {
			...
			else
			{
                ...
                {
                    PrintRecordService.GetInstance().StartRecord(job);
                }
				if(jobItem.Selected)
				{
					UpdateButtonsStates();
...
//			m_StartTime = DateTime.Now;
        ...
        public void OnChangedCalcuTime()
        {
            m_PreviewAndInfo.OnPrintingEnd();
        }
        ...
        {
            m_PrintJobTask.CurrentJobPrintPercent = percent;
        }

	    public bool IsCurrentJobPrintFinish()
	    {
	        return m_PrintJobTask.CurrentJobPrintPercent==100;
        }
		...
		public void OpenJob()
		{
		    ...
		    {
                AddDoubleFourColorJobForm addform = new AddDoubleFourColorJobForm();
                addform.StartPosition = FormStartPosition.CenterScreen;
                //addform.Owner = this;
                if (addform.ShowDialog() == DialogResult.OK)
                {
                    this.AddJobDouble4ColorJob(addform.FileName1, addform.FileName2);
                }
            }
		    else
		    {
		...
		public void AddJobDouble4ColorJob(string file1,string file2)
		{
			try
			{
				UIJob	job	= new UIJob();
                SJobSetting_UI jobset = job.sJobSetting;
				jobset.bReversePrint = this.m_iPrinterChange.GetAllParam().PrinterSetting.sBaseSetting.bReversePrint;
			    jobset.bIsDouble4CJob = true;
                job.sJobSetting = jobset;

				job.Name	= Path.GetFileName(file1);
				job.PreViewFile = PreviewFolderPath + Path.GetFileNameWithoutExtension(file1) + ".bmp";
				job.Status	= JobStatus.Idle;
				SPrtFileInfo	jobInfo = new SPrtFileInfo();
				Int32 bret = 0;
				bret = CoreInterface.Printer_GetFileInfo(file1,ref jobInfo,0);
				if(bret == 1)
				{
					job.PrtFileInfo	= jobInfo;
                    job.FileLocation = file1;
                    job.FileLocation2 = file2;
					job.sJobSetting = jobset;
				}
				else
				{
		...
		private void AddJob(string fileName,bool bPrint,bool bReversePrint)
		{
            ...
            {
                m_JobListView.Sort();
                foreach (ListViewItem item in m_JobListView.SelectedItems)
                {
                    item.Selected = false;
                }
                m_JobListView.Items[0].Selected = true;
        ...
        private void PrintJob(bool blevellingPrint)
		{
            ...
            LogWriter.SaveOptionLog(string.Format("PrintJob printableCount={0};m_JobListView.SelectedItems.Count={1}", printableCount, m_JobListView.SelectedItems.Count));
	    ...
	    public void PrintJob()
	    {
	        try
	        {
                ...
                if (0 == m_JobListView.SelectedItems.Count)
                {
                    //未选中Job弹出提示框
                    string info = ResString.GetEnumDisplayName(typeof(Confirm), Confirm.Print);
                    MessageBox.Show(info, ResString.GetProductName(), MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }

                CoreInterface.IsAbortJob = false;

	            LogWriter.SaveOptionLog("PrintJob() called!!! ");
                ...
                LogWriter.SaveOptionLog("PrintJob() called 1!!! ");
	            ...
	            LogWriter.SaveOptionLog("PrintJob() called 2!!! ");
                if (!CheckMediaHeaterTemp())
	            {
	                return;
	            }
                LogWriter.SaveOptionLog("PrintJob() called 3!!! ");
                this.PrintJob(false);
	        }
	        catch (Exception ex)
	        {
	    ...
	    private void PrintJob(UIJob job)
		{
            ...
            if (CoreInterface.LayoutIndex >= 0)
                LayoutIdx = CoreInterface.LayoutIndex;
            NewLayoutFun.GetLayoutSetting(LayoutIdx, ref curLayoutSetting);
            if (PubFunc.IsAwBMode(curLayoutSetting) && !job.sJobSetting.bIsDouble4CJob)
            {
                MessageBox.Show(ResString.GetResString("Software_AWB_JobSettingError"),ResString.GetProductName(),MessageBoxButtons.OK);
                return;
            }
            // 禁用作业独立的反向打印设置,如需实现作业不同,使用打印模式替换,否则打印模式和作业独立逻辑冲突
	        job.sJobSetting.bReversePrint = m_iPrinterChange.GetAllParam().PrinterSetting.sBaseSetting.bReversePrint;
            ...
            m_PrintJobTask.IsUsedJobMode = m_IsUsedJobMode;
            m_PrintJobTask.IsUsedJobMediaMode = m_IsUsedJobMediaMode;
            m_PrintJobTask.JobMode = m_JobMode;
            m_PrintJobTask.JobMediaMode = m_JobMediaMode;
        ...
        public bool Is3DJobPrintFinish()
        {
	        if (PubFunc.Is3DPrintMachine())
            {
                if (m_PrintJobTask.IsWorking())
                {
                    if (!IsCurrentJobPrintFinish())
                    {
                        string errorInfo = SErrorCode.GetResString("Software_ContinueAbort");
                        ...
                        m_PrintJobTask.CurrentJobPrintPercent = 100;
                        return true;
                    }
                    else
                    {
		...
		private void UpdateButtonsStates()
		{
            ...
            menuItemLLPrint.Enabled = m_MenuItemPrint.Enabled = jobOperate.CanPrintJob && m_IsFATAL && keepwetButtonCan;
		...
		private void m_ContextMenuJobList_Popup(object sender, System.EventArgs e)
		{
            ...
            m_MenuItemPrint.Enabled = jobOperate.CanPrintJob && !BYHXSoftLock.m_DongleKeyAlarm.IsILLEGALDOG && m_IsFATAL && keepwetButtonCan;
...
#endif
				{
					try
					{
					    ...
					    if (this.InvokeRequired)
                            this.Invoke(new AddJobHandler(this.AddJob), new object[] { jobpath, EpsonLCD.CheckHasPaper() && m_Preference.HotFolderPrintImmediately });
                        else
                            this.AddJob(jobpath, EpsonLCD.CheckHasPaper() && m_Preference.HotFolderPrintImmediately);
        ...
        {
            return this.m_PrintJobTask.PrintingPage;
        }
            ...
            else
            {
                ...
                if (File.Exists(PubFunc.GetFullPreviewPath(path)))
                {
                    ...
                    try
                    {
                        ...
                        if (UIFunctionOnOff.PreviewRotate180 && !job.IsClipOrTile)
                        {
        ...
        public void PrintJobWithInkQuantity(string parameter)
        {
            int count = m_JobListView.Items.Count;
            for (int i = count - 1; i >= 0; i--)
            {
                ...
                if (index > 0)
                {
                    string filename = parameter.Substring(0, index);
                    string cNegMaxGrayStr = parameter.Substring(index + 1, parameter.Length - index - 1);
                    byte cNegMaxGray;
                    if (byte.TryParse(cNegMaxGrayStr, out cNegMaxGray))
                    {
                        if (((UIJob)jobItem.Tag).FileLocation == filename)
                        {
                            ((UIJob)jobItem.Tag).sJobSetting.cNegMaxGray = cNegMaxGray;
                            PrintJob((UIJob)jobItem.Tag);
                        }
                    }
                }
        ...
        public MeasureQuestionForm m_MeasureQuestionForm = null;
        public DialogResult ShowMeasureQuestionForm(bool mainformloaded, bool isMeasureBeforePrint = false, bool IsEp6 = false)
        {
                ...
                )// PubFunc.IsVender92())
            {
                if (m_MeasureQuestionForm == null)
                {
                    float tempThick = m_allParam.PrinterSetting.sBaseSetting.fPaperThick;
                    float tempZSpace = m_allParam.PrinterSetting.sBaseSetting.fZSpace;
                    ...
                    {
                        dr = m_MeasureQuestionForm.ShowDialog();
                        ...
                        if (m_MeasureQuestionForm.HasMeasured
                            || m_allParam.PrinterSetting.sBaseSetting.fPaperThick != tempThick
                            || m_allParam.PrinterSetting.sBaseSetting.fZSpace != tempZSpace
                            )
                            UpdateZWorkPosToBoard();
                        CoreInterface.SetPrinterSetting(ref m_allParam.PrinterSetting, true);
                        m_MeasureQuestionForm = null;
                    }
                    return dr;
                }
        ...
        private void UpdateZWorkPosToBoard()
        {
            ...
            if (ScorpionCoreInterface.GetByhxZMoveParam(out param))
            {
                AllParam m_allParam = m_iPrinterChange.GetAllParam();
                float zWorkPos = 0;
                float zMaxPos1 = m_allParam.PrinterSetting.sExtensionSetting.zMaxLength;
                if (zMaxPos1 <= 0)
                {
                    MessageBox.Show("zMaxLength is zero!!");
                    return;
                }
                float fZSpace = m_allParam.PrinterSetting.sBaseSetting.fZSpace + m_allParam.PrinterSetting.sBaseSetting.fPaperThick;
                zWorkPos = zMaxPos1 - fZSpace;
                if (zWorkPos < 0)
                    zWorkPos = 0;
                param.Flag = BitConverter.ToUInt32(new byte[] { (byte)'B', (byte)'Y', (byte)'H', (byte)'X' }, 0);
                param.HeadToPaper =
                    (uint)
                        (zWorkPos * m_allParam.PrinterProperty.fPulsePerInchZ);
                param.activeLen = 8;
                param.PaperThick = 0;
                ScorpionCoreInterface.SetByhxZMoveParam(param);
                LogWriter.SaveOptionLog(string.Format("JoblistView zWorkPos={0};CleanPlace={1};WetPlace={2};fPulsePerInchZ={3},fMesureMaxLen={4},", param.HeadToPaper, param.CleanPlace, param.WetPlace, m_allParam.PrinterProperty.fPulsePerInchZ, zMaxPos1));
            }
            else
            {
