...
namespace BYHXPrinterManager.Main
{
    ...
    public partial class MainForm : System.Windows.Forms.Form, IPrinterChange, IMessageFilter
    {
        ...
        private bool m_bFirstCheckVersion = false;
        ...
        private System.Windows.Forms.Timer mKeepwetTimer;
        ...
        private bool isClickPrinting = false;
        ...
        Thread downloadDBThread = null;

        bool XiaShaFlag = false;

        bool ZEnable_Other = false;
        bool WetEnable_Other = false;
        bool IsFactoryUser = false;
        bool keepwetButtonCan = true;
        ...
        public MainForm(bool bshowUI, string skinName = "")
        {
            ...
            mKeepwetTimer = new System.Windows.Forms.Timer();
            mKeepwetTimer.Enabled = false;
            mKeepwetTimer.Interval = 1000; // 10秒
            mKeepwetTimer.Tick += new EventHandler(mKeepwetTimer_Tick);
#if !INKTYPE
            ...
            this.m_ToolBarButtonMotionSetting.Visible = PubFunc.IsMororSettingAdmin();
            ...
            //errorinfo数据库更新
            //CheckVersion.ReplaceDataBase();

            ////下载新数据库
            //downloadDBThread = new Thread(new ThreadStart(CheckVersion.DownNewDataBase));
            //downloadDBThread.Start();

            //下载错误码数据库
            Thread downloadErrDbThreadSqLite = new Thread(new ThreadStart(SQLiteHelper.DownNewDataBase_SQLite)) { IsBackground = true };
            downloadErrDbThreadSqLite.Start();
            //下载头板版本数据库
            Thread downloadHbVersionDbThreadSqLite = new Thread(new ThreadStart(CheckVersion.DownHeadVersionInfoDataBase_SQLite)) { IsBackground = true };
            downloadHbVersionDbThreadSqLite.Start();
        ...
        {
            LogWriter.SaveOptionLog("[StatusChange]" + status.ToString() + " " + DateTime.Now.ToString());

            if (status == JetStatusEnum.Busy)
            {
                isPrinting = true;
                isClickPrinting = true;
            }
            else
            {
                ...
                {
                    isPrinting = false;
                    isClickPrinting = false;
                }
            }

            if (status == JetStatusEnum.PowerOff && PubFunc.IsSupportKeepwet())
            {
                mKeepwetTimer.Stop();
            }
            ...
            {
#if ADD_HARDKEY
                //if (SPrinterProperty.IsGongZeng() || SPrinterProperty.IsDocanPrintMode() || SPrinterProperty.IsRuiZhi() || SPrinterProperty.IsTateKey()) // 此功能按 厂商限定
                if(true) // 默认加密狗的逻辑都是一狗一板的逻辑
                {
                    ...
                    if (ret == BYHX_SL_RetValue.SUCSESS)
                    {
                        ...
                        {
                            MessageBox.Show(ResString.GetResString("EncryptDog_MisBoardId"));
                        }
                    }
                }
                else
            ...
            {
                if (m_bFirstCheckVersion == false)
                {
                    m_bFirstCheckVersion = true;
                    CheckVersion.CheckHBVersion_New();
                }
                ...
                if ((m_allParam.PrinterProperty.IsDocan() || m_allParam.PrinterProperty.IsColorJetRoolUv()) && !SPrinterProperty.IsLingFeng_RollUV_16H()
                    && m_allParam.PrinterProperty.bSupportZendPointSensor && !PubFunc.IsDocan_Belt_Textile()
                    && !PubFunc.IsDocan_Belt() && !PubFunc.IsDOCAN_FLAT_UV())
                {
                    if (userInfo.motorDebug != 1)
                    {
                        ...
                        float fMeshHight = 0;
                        if (fPaperThick + fMeshHight > 0 && m_allParam.PrinterProperty.fPulsePerInchZ > 0)// 如果移动值为0则表示一直移动
                            CoreInterface.MoveZ(01, fZSpace, fPaperThick + fMeshHight);
                        LogWriter.SaveOptionLog(string.Format(
                            "move z to work pos after ready; fZSpace={0};fPaperThick ={1};fPulsePerInchZ={2}", fZSpace,
                            fPaperThick, m_allParam.PrinterProperty.fPulsePerInchZ));
                    }
                }
                ...
                if (SPrinterProperty.IsLingFeng_RollUV_16H())
                {
                    ...
                    {
                        //ZEnable_Other = lfPrarm.ZEnable == 1 ? true : false;
                        WetEnable_Other = lfPrarm.WetEnable == 1 ? true : false;

                        //UpdateButtonStates(CoreInterface.GetBoardStatus());
                    }
                }
                //GMA喷头第一次就绪时检查头板软件版本号，低于指定版本（v4.1.0.3前三位）提示版本过低（原因：喷头映射关系不同）
                if (m_allParam.PrinterProperty.ePrinterHead == PrinterHeadEnum.Fujifilm_GMA9905300_5pl ||
                    m_allParam.PrinterProperty.ePrinterHead == PrinterHeadEnum.Fujifilm_GMA3305300_5pl)
                {
                    SBoardInfo sBoardInfo = new SBoardInfo();
                    if (CoreInterface.GetBoardInfo(0, ref sBoardInfo) != 0)
                    {
                        SFWVersion fwv = new SFWVersion(sBoardInfo.m_nHBBoardVersion);
                        if (!(fwv.m_nMainVersion >= 4 && fwv.m_nSubVersion >= 1 && fwv.m_nBuildVersion >= 0))
                        {
                            MessageBox.Show(ResString.GetResString("HB_Version_Low"), this.Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                    }
                }
                ...
                {
                    //this.dirForward = MoveDirectionEnum.Down_Z4;
                    //this.dirBackward = MoveDirectionEnum.Up_Z4;
                    EpsonLCD.SetWorkPosInfo(this.m_allParam.PrinterSetting);
                }
                if (PubFunc.IsSupportUVOffsetDistance())
                {
                    UVOffsetDistanceUI uvOffset = m_allParam.ExtendedSettings.UVOffsetDis;
                    EpsonLCD.SetUVOffsetDistToFw(uvOffset, m_allParam.PrinterProperty.fPulsePerInchX);
                }
            }
        ...
        private void UpdateZMaxLength()
        {
            if (CoreInterface.CurBoardStatus != JetStatusEnum.Ready)
                return;
            // 如果支持z终点传感器,则从fw获取z最大行程
            if (m_allParam.PrinterProperty.bSupportZendPointSensor)
            {
                ...
                if (ret == 1 && m_allParam.PrinterProperty.fPulsePerInchZ > 0 && zMax > 0)
                {
        ...
        public void OnPrinterPropertyChange(SPrinterProperty sp)
        {
            ...
            m_MenuItemFactoryTool.Visible = !SPrinterProperty.IsGongZeng();
            if (bFacUser)
            {
            ...
            bool bNoRealPage =
                ...
                || (sp.ePrinterHead == PrinterHeadEnum.Epson_5113)
                || (sp.ePrinterHead == PrinterHeadEnum.EPSON_I3200)
                || (sp.ePrinterHead == PrinterHeadEnum.EGen5)
                //|| (sp.ePrinterHead == PrinterHeadEnum.Ricoh_Gen6)
                );
            if (bNoRealPage)
            {
            ...
            //if (hbType == HEAD_BOARD_TYPE.Ricoh_Gen6_4H)//G6老头板用喷头参数,新头板走电压校正
            //{
            //    menuItemPrinterParameter.Visible = true;
            //}
            //else
            //{
            menuItemPrinterParameter.Visible = false;
            //}
            ...
            if (UIFunctionOnOff.OnlyFacUserShowHwSimple && !bFacUser)
            {
            ...
            this.m_MenuItemCalibraion.Visible = !sp.EPSONLCD_DEFINED || bFacUser;
            ...
            this.m_ToolBarButtonSand.Visible = PubFunc.Is3DPrintMachine(); //&& (PubFunc.IsFhzl3D() || PubFunc.IsLongYuan());
            ...
            m_MenuItemPassword.Visible = !SPrinterProperty.IsInwearSimpleUi() || PubFunc.GetUserPermission()==(int)UserPermission.SupperUser;
            menuItemDoubleYAxisSetting.Visible = UIFunctionOnOff.SupportDoubleYAxis && !SPrinterProperty.IsDocanPrintMode();
            menuItem_KYWaveform.Visible = (SPrinterProperty.IsKyocera300And600(sp.ePrinterHead) && !SPrinterProperty.IsFlora()) ||//彩神不要这个功能
                sp.ePrinterHead == PrinterHeadEnum.Konica_KM1024A_6_26pl ||
                sp.ePrinterHead == PrinterHeadEnum.Epson_5113
                || sp.ePrinterHead == PrinterHeadEnum.EPSON_I3200
				|| sp.ePrinterHead == PrinterHeadEnum.Epson_S2840
                || sp.ePrinterHead == PrinterHeadEnum.EPSON_S1600_RC_UV
                || sp.ePrinterHead == PrinterHeadEnum.Ricoh_Gen6
                || sp.ePrinterHead == PrinterHeadEnum.XAAR_1201_Y1;

            if(PubFunc.IsSupportKeepwet())
            {
                mKeepwetTimer.Enabled = true;
                mKeepwetTimer.Start();
            }
        }
        ...
        public void NotifyUIParamChanged()
        {
            ...
            if (m_ToolbarSetting.comboBoxPringConfigList.Text.ToLower() != "default" && m_ToolbarSetting.comboBoxPringConfigList.Text.Trim() != "")
            {
        ...
        private void UpdateButtonStates(JetStatusEnum status)
        {
            ...
            if (PubFunc.IsSupportKeepwet())
            {
                m_ToolbarSetting.btnMoisturize.Enabled = printerOperate.CanClean;
                keepwetButtonCan = (EpsonLCD.GetKeepwat() == 0);
            }
            else
            {
                keepwetButtonCan = true;
            }

            m_toolBarButtonRetractableCloth.Enabled = status == JetStatusEnum.Ready && GZClothMotionParam.enable != 0;
            m_ToolBarButtonMotionSetting.Enabled = status != JetStatusEnum.PowerOff;

            m_LastOperate = printerOperate;

            menuItemPrinterParameter.Enabled = m_MenuItemRealTimeChart.Enabled = m_MenuItemRealTime.Enabled = printerOperate.CanSaveLoadSettings;
            ...
            m_MenuItemCalibraion.Enabled = true;// printerOperate.CanUpdate;
            menuItem_KYWaveform.Enabled = printerOperate.CanSaveLoadSettings;

            if (status == JetStatusEnum.Busy || status == JetStatusEnum.PowerOff)
                m_MenuItemUpdateLayout.Enabled = false;
            else
                m_MenuItemUpdateLayout.Enabled = true;
            //m_MenuItemInkStatistics.Enabled = printerOperate.CanSaveLoadSettings;

            m_ToolBarButtonPrint.Enabled = m_MenuItemPrint.Enabled = printerOperate.CanPrint && m_MenuItemCalibraion.Enabled && !m_bFrobidPrinter && m_IsFATAL;//&& keepwetButtonCan;
            ...
            m_ToolBarButtonLeft.Enabled = printerOperate.CanMoveLeft && keepwetButtonCan;
            m_ToolBarButtonRight.Enabled = printerOperate.CanMoveRight && keepwetButtonCan;
            ...
            m_ToolBarButtonGoHome.Enabled = printerOperate.CanMoveLeft && printerOperate.CanMoveRight && keepwetButtonCan;
            ...
            m_ToolBarButtonSetOrigin.Enabled = printerOperate.CanPrint && keepwetButtonCan;
            ...
            m_ToolBarButtonZero.Enabled = SPrinterProperty.IsLingFeng_RollUV_16H() ? (WetEnable_Other && printerOperate.CanOffline) : printerOperate.CanOffline;
        ...
        private void OnSingleClean()
        {
            bool bColorJet = PubFunc.IsColorJet_Belt_Textile();
            if (bColorJet ||
                PubFunc.IsGzSurrportManualClean(m_allParam.PrinterProperty) ||
                PubFunc.IsALLWIN_ROLL_TEXTILE() ||
                PubFunc.IsDocan_Belt_Textile() ||
                PubFunc.IsHuiLiCai() ||
                PubFunc.IsPuQi() ||
                PubFunc.IsBoHao())
            {
        ...
        private void OnPrinterPropertyChange_Toolbar(SPrinterProperty sp)
        {
            ...
            if (sp.bSupportZMotion || SPrinterProperty.IsLingFeng_RollUV_16H())
            {
            ...
            m_ToolBarButtonZero.Visible = SPrinterProperty.IsSurportCapping() || SPrinterProperty.IsLingFeng_RollUV_16H(); // !isSkyShip;//印可丽已经将此功能移到panel上了
                ...
                case CoreMsgEnum.ErrorCode:
                    {
                        ...
                        if (SErrorCode.IsOnlyPauseError(errorCode))
                        {
                            ...
                            else
                            {
                                ...
                                else if ((sErrorCode.nErrorCause == (byte)ErrorCause.COM && sErrorCode.n16ErrorCode == (short)COMCommand_Abort.TouchSensor))
                                {
                                    if (isPrinting)
                                    {
                                        result = m_MyMessageBox.Show(errorInfo, ResString.GetProductName(), MessageBoxButtons.RetryCancel, MessageBoxIcon.Exclamation);
                                    ...
                                    {
                                        result = m_MyMessageBox.Show(errorInfo, ResString.GetProductName(), MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                                        bNeedClearFwError = true;
                                    }
                                }
                                else if ((sErrorCode.nErrorCause == (byte)ErrorCause.CoreBoard &&
                            ...
                            if (0 != sErrorCode.nErrorCode)
                            {
                                if (sErrorCode.n16ErrorCode == 1)
                                {
                                    if (konicTemperatureForm == null &&
                                        ...
                                        Waveform_S2840 == null &&
                                        Waveform_KM1024A == null) //屏蔽gama下载波形重启提示
                                    {
                                    ...
                                    else if (Waveform_KyoceraForm != null)
                                    {
                                        Waveform_KyoceraForm.DownloadEvent.Set();
                                    }
                                    else if (Waveform_S2840 != null)
                                    {
                                        Waveform_S2840.DownloadSuccess = true;
                                        Waveform_S2840.DownloadWaveformAutoResetEvent.Set();
                                    }
                                }
                                else
                                {
                                    if (Waveform_S2840 != null)
                                    {
                                        Waveform_S2840.DownloadSuccess = false;
                                        Waveform_S2840.DownloadWaveformAutoResetEvent.Set();
                                    }
                        ...
                        if ((cause == ErrorCause.COM && (ErrorAction)sErrorCode.nErrorAction == ErrorAction.Pause
                            ...
                            || (cause == ErrorCause.CoreBoard && (ErrorAction)sErrorCode.nErrorAction == ErrorAction.Pause
                                                        && sErrorCode.n16ErrorCode == 0x101) // 缺墨报警
                            )
                        {
                            ...
                            case Ep6Cmd.Panel_PIPE:
                                {
                                    ...
                                    if (CoreInterface.GetDirtyCmd(ep6Cmd, buf, ref buflen) != 0)
                                    {
                                        ...
                                        if (temp[12] != 0)
                                        {
                                            ...
                                            msg += (temp[12] & (1)) != 0 ? ResString.GetResString("ColorNegativePressureError") : "";
                                            msg += (temp[12] & (1 << 1)) != 0 ? ResString.GetResString("WhiteNegativePressureError") : "";
                                            msg += (temp[12] & (1 << 2)) != 0 ? ResString.GetResString("SafetyCylinder1FeedInk") : "";
                                            msg += (temp[12] & (1 << 3)) != 0 ? ResString.GetResString("SafetyCylinder2FeedInk") : "";
                                            msg += (temp[12] & (1 << 4)) != 0 ? ResString.GetResString("LowExternalPressure") : "";
                                            msg += (temp[12] & (1 << 5)) != 0 ? ResString.GetResString("HighExternalPressure") : "";
                                    ...
                                    {
                                        MessageBox.Show(string.Format(ResString.GetResString("GetDirtyCmdFialed"), ep6Cmd));
                                    }
                                    break;
                                }
                            ...
                            case Ep6Cmd.LowInkBit:
                                {
                                    ...
                                    if (CoreInterface.GetDirtyCmd(ep6Cmd, buf, ref buflen) != 0)
                                    {
                                        ...
                                        if (!string.IsNullOrEmpty(colorName))
                                            errStr = string.Format("{0}{1}", colorName, SErrorCode.GetResString("CoreBoard_Fatal_INK_SUPPLY_LOW"));
                                    //this.m_StatusBarPanelError.Text = errStr;
                                        this.m_PrintInformation.PrintJobInfomation(code, UserLevel.user, errStr);
                                }
                                else
                            ...
                            case Ep6Cmd.Vender_Message:
                                {
                                    ...
                                    {
//#if DEBUG
//                                        string temp = "收到0x114Ep6命令:" ;
//                                        for(int i=0;i< 8;i++)
//                                        {
//                                            temp += buf[i].ToString("X") + ",";
//                                        }
//                                        MessageBox.Show(temp);
//#endif
                                        if (buflen >= 4)
                                        {
                                            ushort MsgType = BitConverter.ToUInt16(buf, 2);

                                            switch (MsgType)
                                            {
                                                case 0x01: //华翔3D，移动到位，可以下沙
                                                    LogWriter.SaveOptionLog(string.Format("[TCP] BeginXiaSha"));

                                                    ProtocolByhx returnFhzl = new ProtocolByhx(TcpIpCmdByhx.ResponsePackage, 0x40, "");
                                                    remoteControlServer.SendCmdReturnValue(returnFhzl);
                                                    XiaShaFlag = true;
                            ...
                            case Ep6Cmd.IO_State:
                                {
                                    LogWriter.WriteLog(new string[] { "IO_State" }, true);
                                    byte ep6Cmd = (byte)lParam.ToInt64();
                                    byte[] buf = new byte[512];
                                    int buflen = buf.Length;
                                    if (CoreInterface.GetDirtyCmd(ep6Cmd, buf, ref buflen) != 0)
                                    {
                                        if (buflen >= 4)
                                        {
                                            ...
                                            if (buf[0] == 0x82 && buf[3] == 0x09)// mb io,j32
                                            {
                                                if (PubFunc.IsHapond())
                                                {
                                                    ShowHapondIoChangeMsg(buf);
                                                }
                                            }
                                            if (buf[0] == 0x82 && buf[3] == 0x08)//dsp io
                                            {
                                                if (factroyToolForm != null)
                                                {
                                                    factroyToolForm.IOStatusRefresh(buf);
                                                }
                                            }
                                        }
                                    }
                ...
                case CoreMsgEnum.HardPanelCmd:
                    {
                        ...
                        switch ((HardPanelCmd)lParam.ToInt64())
                        {
                            ...
                            case HardPanelCmd.PRINT_NOZZLE_CHECK:
                                if (!isClickPrinting || clickTime.AddSeconds(2) < DateTime.Now)
                                {
                                    isClickPrinting = true;
                                    clickTime = DateTime.Now;
                                SPrinterSetting ss = m_allParam.PrinterSetting;
                                ss.sFrequencySetting.nResolutionX /= ss.sBaseSetting.nXResutionDiv;
                                CoreInterface.SendCalibrateCmd((int)CalibrationCmdEnum.CheckNozzleCmd, 0, ref ss, true);
                                }
                                break;
                            ...
                            case HardPanelCmd.FROBIDPRINT:
                                ...
                                if (!m_ToolBarButtonAbort.Enabled)
                                {
                                    m_JobListForm.TerminatePrintingJob(false);
                                    this.m_bFrobidPrinter = true;
                                    MessageBox.Show(ResString.GetResString("PrintSwitchHasBeenTurnedOff"));
                                }
                            ...
                            case HardPanelCmd.PRINT_START_CS:
                                ...
                                if (m_ToolBarButtonPrint.Enabled)
                                    m_JobListForm.PrintJob();
                                break;
                            case HardPanelCmd.MeasureHeight:

                                JetStatusEnum printerStatus = CoreInterface.GetBoardStatus();
                                if (printerStatus == JetStatusEnum.Ready)
                                {
                                    m_JobListForm.ShowMeasureQuestionForm(true, false, true);
                                }
                ...
                case CoreMsgEnum.PrintInfo:
                    ...
                    break;
                case CoreMsgEnum.FinishedBand: //打印完成的band索引
                    {
                        //int finishedBandindex = (int)lParam.ToInt64();
                    }
                    break;
                            ...
                            if (CoreInterface.GetEp6PipeData(ep6Cmd, index, bufep6, ref buflen) != 0)
                            {
                                switch (bufep6[0])
                                {
                                    case 0xE3:
                                        if (hwForm != null)
                                        {
                                            hwForm.SetTemperaturProfileState(bufep6);
                                        }
                                        break;
                                    case 0xDE:
                                        if (hwForm != null)
                                        {
                                            hwForm.SetTemperaturProfile(bufep6[1]);
                                        }
                                        break;
                                    default:
                                        if (hwForm != null)
                                        ...
                                        if (Waveform_KyoceraForm != null)
                                        {
                                            Waveform_KyoceraForm.OnEp6DataChanged(ep6Cmd, index, bufep6);
                                        }
                                        break;
                                }
                                if (_factoryToolForm2 != null)
                                {
                                    _factoryToolForm2.CallBack(bufep6);
                                }
                            }
                        }
                        break;
        ...
        private void OnRecivePowerOnEventMsg()
        {
        ...
        public bool End()
        {
            try
            {
                ...
                if ((SPrinterProperty.IsGongZeng() || SPrinterProperty.IsDocanPrintMode() || SPrinterProperty.IsRuiZhi()) && InkAreaStaticsHelper.CanSynchronizeInkAndArea() && !InkAreaStaticsHelper.BoardHasChanged())
                {
        ...
        public void OnErrorCodeChanged(int code)
        {
            ...
            if (errcode.nErrorAction == (byte)ErrorAction.Warning && (errcode.n16ErrorCode == (short)0x68 || errcode.n16ErrorCode == (short)0x67)) //屏蔽波形斜率超限警告
            {
                IsShowWarning = false;
            }

            if (IsFactoryUser || IsShowWarning)
            {
                if (bColorChange)
                    errStr += "< ... 2 >";
                this.m_StatusBarPanelError.Text = errStr;
        ...
        public void OnPrintingEnd()
        {
            try
            {
                ...
                if (PubFunc.IsSupportHistoryRecord())
                {
                    if (CoreInterface.IsAbortJob)
                    {
                        m_PreviewAndInfo.SaveDataBase(m_PrintPercent);
                    }
                    else
                    {
                        m_PreviewAndInfo.SaveDataBase(100);
                    }
                }
        ...
        private void CalcuJobPrint()
        {
            ...
            if (job.AllCopiesTime != TimeSpan.Zero)
                PrintRecordService.GetInstance().CalculateInk(job, printedWidth, PrintedCopies, m_JobListForm.PrintingPage(), _printPercent,m_allParam.PrinterSetting);
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            if (m.Msg == this.m_KernelMessage)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    LogWriter.WriteLog(new string[] { string.Format("[ProceedKernelMessage] Error:{0}", ex.StackTrace) }, true);
                }
            }
        ...
        private void MainApp_Closing(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            if (downloadDBThread != null && downloadDBThread.ThreadState == System.Threading.ThreadState.Running)
            {
                downloadDBThread.Abort();
            }
        }
        ...
        {
            IsFactoryUser = PubFunc.IsFactoryUser();
#if ADD_HARDKEY
        ...
        private void m_MenuItemAbout_Click(object sender, System.EventArgs e)
        {
            ...
            if (m_sFormTile != "")
            {
                printHeadInfo = this.Text.Replace(m_sFormTile, "");
            }
            AboutForm aboutForm = new AboutForm(printHeadInfo);
        ...
        FactoryTool factroyToolForm = new FactoryTool();
        private void m_MenuItemFactoryTool_Click(object sender, System.EventArgs e)
        {
            factroyToolForm = new FactoryTool();
            factroyToolForm.ShowDialog();
            factroyToolForm.Dispose();
            EpsonLCD.GetGZClothMotionParam(ref GZClothMotionParam);
            UpdateButtonStates(CoreInterface.GetBoardStatus());
        }
        FactoryTool2 _factoryToolForm2 = new FactoryTool2();
        private void m_MenuItemFactoryTool2_Click(object sender, System.EventArgs e)
        {
            _factoryToolForm2 = new FactoryTool2();
            _factoryToolForm2.ShowDialog();
            _factoryToolForm2.Dispose();
        }
        ...
        private void m_MenuItemRealTime_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                if (konicTemperatureForm.ShowDialog(this) == DialogResult.OK)
                {
                    ...
                    CoreInterface.SetPrinterSetting(ref m_allParam.PrinterSetting);
                    LogWriter.LogSetPrinterSetting(m_allParam.PrinterSetting, m_bDuringPrinting,
                        "m_MenuItemRealTime_Click");
        ...
        private void m_MenuItemUVSetting_Click(object sender, System.EventArgs e)
        {
            ...
            if (dr == DialogResult.OK)
            {
                m_hwForm.OnGetPrinterSetting(ref this.m_allParam.PrinterSetting, ref this.m_allParam.ExtendedSettings);
                CoreInterface.SetPrinterSetting(ref this.m_allParam.PrinterSetting);
            }
            m_hwForm = null;
        }
        ...
        private void m_ToolBarCommand_ButtonClick(object sender, System.Windows.Forms.ToolBarButtonClickEventArgs e)
        {
            ...
            else if (e.Button == m_ToolBarButtonMotionSetting)
            {
                MotionSetting_DY msdy = new MotionSetting_DY();
                msdy.ShowDialog();
            }
            ...
            else if (e.Button == toolBarButtonSwitchInk)
            {
                ...
                if (PubFunc.GetCurInkType(ref selectedInkType))
                {
                    ...
                    {
                        MessageBox.Show(string.Format(ResString.GetResString("SwitchInkTypeError"), 0x68, 0x02));
                        return;
                    }
                    else if (ret == -2)
                    {
                        MessageBox.Show(string.Format(ResString.GetResString("UpdatePrinterSettingError"), 0x04));
                    }
                }
                else
                {
            ...
            {
                var result = MessageBox.Show(ResString.GetResString("PowerOffMachine"), this.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
            ...
            else
            {
                ...
                if (e.Button == m_ToolBarButtonCheckNozzle)
                {
                    if (!isClickPrinting || clickTime.AddSeconds(2) < DateTime.Now)
                    {
                        isClickPrinting = true;
                        clickTime = DateTime.Now;
                    m_ToolBarButtonCheckNozzle.Enabled = false;
                ...
                {
                    //聆风京瓷-belt-textile自动清洗功能关联单头清洗功能(ColorjetManualCleanSetting2界面)里的start功能，包括参数
                    if (SPrinterProperty.IsLingFengJingCiBeltTextile())
                    {
                        byte color = 0xff;
                        byte? level = null;
                ...
                else if (e.Button == m_ToolBarButtonStop)
                {
                    ...
                    {
                        CoreInterface.SendJetCommand((int)JetCmdEnum.StopMove, cmdvalue);
                    }
                    return;
        ...
        private void UpdateFormHeaderText()
        {
            ...
            if (status != JetStatusEnum.PowerOff && !isInwearSimpleUi) // inwear精简ui不显示几色几组什么喷头20170523gzw
            {
                ...
                int headNum = NewLayoutFun.GetHeadNum();
        ...
        public void LogPrintedArea(float percent)
        {
        ...
        private string GetPrintedAreaLogString()
        {
            ...
            float width = 0;
            if (m_PrintingJob != null && m_PrintingJob.PrtFileInfo.sFreSetting.nResolutionX != 0)
            {
                width = m_PrintingJob.JobSize.Width;
                width = PubFunc.CalcRealJobWidth(width, this.m_allParam);
            }
            float height = 0;
            if (m_PrintingJob != null && m_PrintingJob.PrtFileInfo.sFreSetting.nResolutionY != 0)
            {
                height = m_PrintingJob.JobSize.Height;
            }
            float fArea = UIPreference.ToDisplayLength(UILengthUnit.Meter, width) * UIPreference.ToDisplayLength(UILengthUnit.Meter, height);
            ...
            string strArea = (fArea * m_PrintPercent / 100.0f).ToString() + " " + unitStr + "2";
            string strFileLocation = m_PrintingJob != null ? m_PrintingJob.FileLocation : string.Empty;
            return string.Format("{0}; {1}; {2}; {3}; {4}", new object[] { strFileLocation, strStartTime, strTime, strPercentage, strArea });
        }
        ...
        private void menuItemPrintRecord_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                explorer.CurrentUnit = m_allParam.Preference.Unit;
                explorer.ShowDialog();
            }
        }
        ...
        private Waveform_KM1024A Waveform_KM1024A;
        private Waveform_S2840 Waveform_S2840;
        private void menuItem_KYWaveform_Click(object sender, EventArgs e)
        {
            if (m_allParam.PrinterProperty.ePrinterHead == PrinterHeadEnum.Konica_KM1024A_6_26pl)
            {
                Waveform_KM1024A = new Waveform_KM1024A();
                Waveform_KM1024A.InitHeadNum();
                Waveform_KM1024A.ShowDialog(this);
                Waveform_KM1024A = null;
            }
            else if (SPrinterProperty.IsKyocera300And600(m_allParam.PrinterProperty.ePrinterHead) && !SPrinterProperty.IsFlora())
            {
            ...
            else if (m_allParam.PrinterProperty.ePrinterHead == PrinterHeadEnum.Epson_S2840 || 
                     m_allParam.PrinterProperty.ePrinterHead == PrinterHeadEnum.Epson_S2840_WaterInk ||
                     m_allParam.PrinterProperty.ePrinterHead == PrinterHeadEnum.Epson_5113 ||
                m_allParam.PrinterProperty.ePrinterHead == PrinterHeadEnum.EPSON_S1600_RC_UV ||
                     m_allParam.PrinterProperty.ePrinterHead == PrinterHeadEnum.EPSON_I3200 ||
                     m_allParam.PrinterProperty.ePrinterHead == PrinterHeadEnum.Ricoh_Gen6 ||
                     m_allParam.PrinterProperty.ePrinterHead == PrinterHeadEnum.XAAR_1201_Y1)
            {
                Waveform_S2840 = new Waveform_S2840();
                Waveform_S2840.ShowDialog(this);
                Waveform_S2840 = null;
            }
        }
        private void menuItemLayoutSetting_Click(object sender, EventArgs e)
        {
            LayoutSettingConfig lsc = new LayoutSettingConfig();
            lsc.ShowDialog();

            m_ToolbarSetting.LayoutSetting_Bind();
        }

        private void m_MenuItemUpdateLayout_Click(object sender, EventArgs e)
        {
            string layoutFileName = "test.plb";
            ...
            try
            {
                ...
                fileDialog.Multiselect = false;
                fileDialog.CheckFileExists = true;
                fileDialog.DefaultExt = ".plb";
                fileDialog.Filter = ResString.GetEnumDisplayName(typeof(FileFilter), FileFilter.Plb);
                fileDialog.InitialDirectory = m_allParam.Preference.WorkingFolder;
                if (fileDialog.ShowDialog(this) == DialogResult.OK)
                {
                    if (File.Exists(fileDialog.FileName))
                    {
                        ...
                        File.Copy(fileDialog.FileName, layoutFileName);

                        MessageBox.Show(ResString.GetResString("LayoutUpdateSuccess"));
                    }
                    else
                    {
                        MessageBox.Show("File not found:" + fileDialog.FileName);
                    }
        ...
        private void mKeepwetTimer_Tick(object sender, EventArgs e)
        {
            bool CurKeepwetButtonCan = EpsonLCD.GetKeepwat() == 0;
            if (!CurKeepwetButtonCan)
            {
                m_ToolbarSetting.btnMoisturize.Text = ResString.GetResString("CancelMoisturizing");
            }
            else
            {
                m_ToolbarSetting.btnMoisturize.Text = ResString.GetResString("Moisturize");
            }

            if (CurKeepwetButtonCan != keepwetButtonCan)
            {
                keepwetButtonCan = CurKeepwetButtonCan;
                UpdateButtonStates(CoreInterface.GetBoardStatus());
            }
        }

        private void menuItemPrinterParameter_Click(object sender, EventArgs e)
        {
            try
            {
                HeadParameterForm Form = new HeadParameterForm();
        ...
        #region 海邦达定制io提示消息
        bool[] _hapondMbIoBits = new bool[5]; // hapond当前主板io状态 j32-6 to j32-9
        /// <summary>
        /// 重置主板io初始值
        /// </summary>
        private void ResetMbIoStatus()
        {
            _hapondMbIoBits = new bool[] { false, false, false, false, false };
        }
        /// <summary>
        /// 显示厂商定制提示信息
        /// </summary>
        /// <param name="buf">ep6信息</param>
        private void ShowHapondIoChangeMsg(byte[] buf)
        {
            // 上报的有5个IO口状态,unsigned char pinInput[] = {J32-5,J32-6,J32-7,J32-8,J32-9}
            // 数据的第1个字节前5个bit位代表这5个IO的状态，1为高电平，0为低电平。
            byte ioBit = buf[7];
            bool[] newMbIoBits = new bool[5];
            newMbIoBits[0] = (ioBit & (1)) > 0;
            newMbIoBits[1] = (ioBit & (1 << 1)) > 0;
            newMbIoBits[2] = (ioBit & (1 << 2)) > 0;
            newMbIoBits[3] = (ioBit & (1 << 3)) > 0;
            newMbIoBits[4] = (ioBit & (1 << 4)) > 0;
            if (newMbIoBits[0] != _hapondMbIoBits[0]) //J32-5
            {
                _hapondMbIoBits[0] = newMbIoBits[0];
            }
            if (newMbIoBits[1] != _hapondMbIoBits[1]) //J32-6 
            {
                _hapondMbIoBits[1] = newMbIoBits[1];
            }
            if (newMbIoBits[2] != _hapondMbIoBits[2]) //J32-7 胶辊升起!
            {
                _hapondMbIoBits[2] = newMbIoBits[2];
                if (newMbIoBits[2]) //
                {
                    MyMessageBox messageBox =
                        new MyMessageBox(AutoStopMoveAndUpdateUI, false, false);
                    string errorInfo = ResString.GetResString("HapondIoMsg_RollUp"); //"胶辊升起!";
                    if (!string.IsNullOrEmpty(errorInfo))
                    {
                        messageBox.Show(errorInfo, ResString.GetProductName(),
                            MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
            }
            if (newMbIoBits[3] != _hapondMbIoBits[3]) //J32-8 请检查压墨推拉盖!
            {
                _hapondMbIoBits[3] = newMbIoBits[3];
                if (newMbIoBits[3])
                {
                    MyMessageBox messageBox =
                        new MyMessageBox(AutoStopMoveAndUpdateUI, false, false);
                    string errorInfo = ResString.GetResString("HapondIoMsg_CheckPressInk");//"请检查压墨推拉盖!"; 
                    if (!string.IsNullOrEmpty(errorInfo))
                    {
                        messageBox.Show(errorInfo, ResString.GetProductName(),
                            MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            if (newMbIoBits[4] != _hapondMbIoBits[4]) //J32-9请打开水冷机!
            {
                _hapondMbIoBits[4] = newMbIoBits[4];
                if (newMbIoBits[4])
                {
                    MyMessageBox messageBox =
                        new MyMessageBox(AutoStopMoveAndUpdateUI, false, false);
                    string errorInfo = ResString.GetResString("HapondIoMsg_OpenCooling");  //"请打开水冷机!";
                    if (!string.IsNullOrEmpty(errorInfo))
                    {
                        messageBox.Show(errorInfo, ResString.GetProductName(),
                            MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        public void SetCloseNozzle()
        {
            ...
            if (File.Exists(filePath))
            {
                ...
                while (!sr.EndOfStream)
                {
                    ...
                    dynamicdata.curcolorindex = (byte)NewLayoutFun.GetColorID(strArray[0]);
