    ...
    partial class MainForm
    {
        ...
        private void ProcessGetInfoCmd(ProtocolByhx protocol)
        {
            switch ((GetInfoSubCmd)protocol.SubCmd)
            {
                ...
                //    break;
                //}
                case GetInfoSubCmd.StepHeightPerPass:
                    {
                        int stepPerPass = 0;
                        float InchPerPass = 0;
                        SPrtFileInfo jobInfo = new SPrtFileInfo();
                        if (CoreInterface.Printer_GetJobInfo(ref jobInfo, false) > 0)
                        {
                            stepPerPass = CoreInterface.GetJobAdvanceHeight(jobInfo.sFreSetting.nPass);
                            //InchPerPass = CoreInterface.GetAdvanceHeight(jobInfo.sFreSetting.nPass);

                            InchPerPass = (float)stepPerPass / (float)m_allParam.PrinterSetting.sFrequencySetting.nResolutionY;
                        }
                        //float fstepHeightPerPass = stepPerPass/CoreInterface.GetfPulsePerInchY(0);
                        //int PulseValue = (int)(InchPerPass * CoreInterface.GetfPulsePerInchY(0));
                        ProtocolByhx returnFhzl = new ProtocolByhx(TcpIpCmdByhx.ResponsePackage, protocol.SubCmd,
                            InchPerPass.ToString());
                        remoteControlServer.SendCmdReturnValue(returnFhzl);
                        break;
                    }
                case GetInfoSubCmd.BoardSerialNum:
                    {
                        string info = string.Empty;
                        SBoardInfo sBoardInfo = new SBoardInfo();
                        if (CoreInterface.GetBoardInfo(0, ref sBoardInfo) != 0)
                        {
                            info = sBoardInfo.m_nBoardSerialNum.ToString();
                        }
                        ProtocolByhx returnFhzl = new ProtocolByhx(TcpIpCmdByhx.ResponsePackage, protocol.SubCmd,
                             info);
                        remoteControlServer.SendCmdReturnValue(returnFhzl);
        ...
        private void ProcessOprationCmd(ProtocolByhx protocol)
        {
            switch ((OprationCmdSubCmd)protocol.SubCmd)
            {
                ...
                case OprationCmdSubCmd.PrintJobWithInkQuantity:
                    {
                        m_JobListForm.PrintJobWithInkQuantity(protocol.Parameter);
                        break;
                    }
                ...
                case OprationCmdSubCmd.XiaShaFinish:
                    {
                        LogWriter.SaveOptionLog(string.Format("[TCP] XiaShaFinish"));
                        //if (XiaShaFlag)
                        {
                            XiaShaFlag = false;
        ...
        private void ProcessPrinterSettingCmd(ProtocolByhx protocol)
        {
            ...
            switch ((PrinterSettingSubCmd)protocol.SubCmd)
            {
                ...
                case PrinterSettingSubCmd.NyMoveSpeed:
                    {
                        byte nYMoveSpeed = 0;
                        bool ret = byte.TryParse(protocol.Parameter, out nYMoveSpeed);
                        if (!ret)
                            break;
                        ss.sMoveSetting.nYMoveSpeed = nYMoveSpeed;
                        break;
                    }
                //default:
                //    throw new ArgumentOutOfRangeException();
