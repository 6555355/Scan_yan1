...
//		public static bool bNcLButtonDown = false;
		...
		public static bool LoadRenderResource(string skinPath)
		{
			try
			{
				imgBtnRes = Image.FromFile(Application.StartupPath + "\\Skins\\newBlue\\btnRes.png");
				imgBtnMin = Image.FromFile(Application.StartupPath + "\\Skins\\newBlue\\btnMin.png");
				imgBtnMax = Image.FromFile(Application.StartupPath + "\\Skins\\newBlue\\bntMax.png");
				imgBtnClose = Image.FromFile(Application.StartupPath + "\\Skins\\newBlue\\btnClose.png");
				imgBtnCustom = Image.FromFile(Application.StartupPath + "\\Skins\\newBlue\\tab_scrollbutton.png");
				imgFill = Image.FromFile(Application.StartupPath + "\\Skins\\newBlue\\msn蓝色.jpg");//不透明背景.jpg");//pic_titbg02.gif");

				imgTools = Image.FromFile(Application.StartupPath + "\\Skins\\newBlue\\msn蓝色_tools.jpg");
				imgFrameLeft = Image.FromFile(Application.StartupPath + "\\Skins\\newBlue\\V_border.jpg");
				imgFrameRight = Image.FromFile(Application.StartupPath + "\\Skins\\newBlue\\V_border.jpg");
				imgFrameBottom = Image.FromFile(Application.StartupPath + "\\Skins\\newBlue\\H_border.jpg");
		...
		public static void PaintFormCaption(ref Message m,Form mform,bool isMainForm)
		{
			...
			//按钮位置
			m_rect.X = (int)(mform.Width - xSizeFrame - 4 * szCaptionButton.Width);
			m_rect.Y = ySizeFrame + yBorder;//6
			m_rect.Width = (int)(szCaptionButton.Width);
			m_rect.Height = (int)(szCaptionButton.Height);//20;

			m_rectMin = new Rectangle(m_rect.X + (int)szCaptionButton.Width,m_rect.Y,m_rect.Width,m_rect.Height);
			m_rectMax = new Rectangle(m_rect.X + (int)szCaptionButton.Width * 2,m_rect.Y,m_rect.Width,m_rect.Height);
			m_rectClo = new Rectangle(m_rect.X + (int)szCaptionButton.Width * 3,m_rect.Y,m_rect.Width - xBorder,m_rect.Height);
			...
			switch(m.Msg)    
			{
				...
				{
					m_rect.X = (int)(mform.Width - xSizeFrame - 4 * szCaptionButton.Width);
					m_rect.Y = 0;//ySizeFrame + yBorder;//6
					m_rect.Width = (int)(szCaptionButton.Width);
					m_rect.Height = (int)(szCaptionButton.Height);//20;
		...
		public static bool NCButtonClick(Message m,Form mform,bool isMainForm)
		{
			...
			switch(m.Msg)    
			{
				case 0xA3://WM_NCLBUTTONDBLCLK
				{
					...
					if(isMainForm)
					{
						if(m_rect.Contains(mousePoint) || m_rectMin.Contains(mousePoint) || m_rectMax.Contains(mousePoint))
						{
							return false;
						}
					}
