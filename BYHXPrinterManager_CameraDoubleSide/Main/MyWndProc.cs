...
//		public static bool bNcLButtonDown = false;
		...
		public static bool LoadRenderResource(string skinPath)
		{
			try
			{
                ...
                if (File.Exists(imgpath)) imgBtnMax = Image.FromFile(imgpath);
                imgpath = Path.Combine(skinPath, "btnClose.png");
                if (File.Exists(imgpath)) imgBtnClose = Image.FromFile(imgpath);
                imgpath = Path.Combine(skinPath, "btnCustom.png");
                if (File.Exists(imgpath)) imgBtnCustom = Image.FromFile(imgpath);
                imgpath = Path.Combine(skinPath, "formheader.png");
                if (File.Exists(imgpath)) ImgFormheader = Image.FromFile(imgpath);//不透明背景.jpg");//pic_titbg02.gif");

                imgpath = Path.Combine(skinPath, "toolsbar.png");
                if (File.Exists(imgpath)) imgTools = Image.FromFile(imgpath);
                imgpath = Path.Combine(skinPath, "toolbarSetting.png");
                if (File.Exists(imgpath)) imgToolSetting = Image.FromFile(imgpath);
                imgpath = Path.Combine(skinPath, "V_border_L.png");
                if (File.Exists(imgpath)) imgFrameLeft = Image.FromFile(imgpath);
                imgpath = Path.Combine(skinPath, "V_border_R.png");
                if (File.Exists(imgpath)) imgFrameRight = Image.FromFile(imgpath);
                imgpath = Path.Combine(skinPath, "H_border.png");
                if (File.Exists(imgpath)) imgFrameBottom = Image.FromFile(imgpath);
                imgpath = Path.Combine(skinPath, "background_main.png");
                if (File.Exists(imgpath)) imgbackground_main = Image.FromFile(imgpath);
                imgpath = Path.Combine(skinPath, "background.png");
                if (File.Exists(imgpath)) imgbackground = Image.FromFile(imgpath);
				loadSucessed = true;
			}
			catch(Exception)
			{
