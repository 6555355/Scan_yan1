...
namespace FileBrowser
{
    ...
    public partial class Browser : UserControl
    {
        ...
        private void InitStartUp()
        {
            ...
            else if (!string.IsNullOrEmpty(this.fileView.m_LastPreviewForlder))
            {
                otherStartupDir = this.fileView.m_LastPreviewForlder;
        ...
        private void OnSelectedFolderChanged(SelectedFolderChangedEventArgs e)
        {
            ...
            m_PreviewForlder = this.fileView.m_LastPreviewForlder;
        ...
        /// <summary>
        /// This method will raise the OnUserContextMenuMouseClickEvent.
        /// </summary>
        /// <param name="e">The UserContextMenuClickEventArgs to pass on</param>
        internal void OnUserContextMenuClick(uint index, EventArgs e)
        {
            if (index == 2)
                this.fileView.ReLoadItems();
            if (this.UserContextMenuClick != null)
                UserContextMenuClick(index, e);
        }
        #endregion
            ...
            switch (e.UpdateType)
            {
                case ShellItemUpdateType.Created:
                    #region Created
                    {
                        ...
                        //    ////fileView.bwLoadImages_DoWork(new string[1] { e.NewItem.Path });
        ...
        private bool SetNewPath(ShellItem oldItem, ShellItem newItem)
        {
            ...
            if (oldItem != newItem && newItem.Expand(true, false, Handle))
            {
                ...
                ArrayList sic = new ArrayList();
                foreach (ShellItem si in newItem.SubFiles)
                {
                    if (si.IsFolder)
                        continue;
                    if (Path.GetExtension(ShellItem.GetRealPath(si)) == ".prt" || Path.GetExtension(ShellItem.GetRealPath(si)) == ".prn")
                        sic.Add(si);
                }
                ListViewItem[] newListItemsArray = new ListViewItem[sic.Count];
                string[] subItems = new string[fileView.Columns.Count - 1];
                for (int i = 0; i < newListItemsArray.Length; i++)
                {
                    //ShellItem.SetInfo((ShellItem)sic[i]);
                    newListItemsArray[i] = GetListViewItem(subItems, (ShellItem)sic[i]);
                }
                ...
                //if (Directory.Exists(ShellItem.GetRealPath(selectedItem)))
                //{
                    this.fileView.selectedItem = selectedItem;
                    this.fileView.FolderName = ShellItem.GetRealPath(selectedItem);
                //}
        ...
        { 
            get 
            { 
                 return this.fileView.SelectedItems; 
            } 
        }
        ...
        [Browsable(false)]
        internal JThumbnailView FileView { get { return fileView; } }
        ...
        [Category("Options"),
         Description("Shows or hides the file ListView"),
         DefaultValue(true),
         Browsable(true)]
        public bool ShowFileView
        {
            get { return !browseSplitter.Panel2Collapsed; }
            set { browseSplitter.Panel2Collapsed = !value; }
        }
        ...
        {
            int newSize = navigationBar.Width - navAddressLabel.Bounds.Right - this.toolStripButtonRefresh.Width - 15;
        ...
        private void folderView_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
...
e.KeyData == Keys.Left || e.KeyData == Keys.Right)
        ...
        private void fileView_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
            if (e.KeyData == Keys.Up || e.KeyData == Keys.Down ||
e.KeyData == Keys.Left || e.KeyData == Keys.Right)
            {
