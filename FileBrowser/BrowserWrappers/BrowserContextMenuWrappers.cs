...
namespace FileBrowser
{
    ...
    internal class BrowserLVContextMenuWrapper : NativeWindow
    {
        ...
        /// the default item of the items contextmenu.ItemDoubleClick
        /// </summary>
        void FileView_ItemActivate(object sender, EventArgs e)
        {
            ...
            if (parent != null && br.FileView.SelectedOrder.Count > 0)
            {
                ...
                //else
                {
                    ...
                    br.OnUserContextMenuClick(1, new EventArgs());
                }
            }
        }
        ...
        private void CreateNormalMenu(Point ptInvoke, ListViewHitTestInfo hitTest, MouseEventArgs e)
        {
            ...
            try
            {
                if (ContextMenuHelper.GetIContextMenu(parentShellFolder, pidls, out iContextMenuPtr, out iContextMenu))
                {
                    ...
                    #region Make Menu
                    ShellAPI.InsertMenu(contextMenu, 0, ShellAPI.MFT.BYPOSITION | 0, (uint)CMD_CUSTOM.SpecialView + (uint)1, "Add To Joblist");
                    ShellAPI.InsertMenu(contextMenu, 1, ShellAPI.MFT.BYPOSITION | ShellAPI.MFT.SEPARATOR, 0, string.Empty);
                    #endregion
                    ...
                    {
                        if (selected > (uint)CMD_CUSTOM.SpecialView)
                        {
                            // Ìí¼Óµ½joblist!
                            br.OnUserContextMenuClick(selected - (uint)CMD_CUSTOM.SpecialView, new EventArgs());
                        }
                        else
                        {
        ...
        private void CreateFolderMenu(Point ptInvoke, MouseEventArgs e)
        {
            ...
            try
            {
                ...
                ShellAPI.AppendMenu(contextMenu, 0, (uint)CMD_CUSTOM.SpecialView + (uint)2, "Refresh");
                ...
                if ((uint)selected > (uint)CMD_CUSTOM.SpecialView)
                {
                    br.OnUserContextMenuClick((uint)selected - (uint)CMD_CUSTOM.SpecialView, new EventArgs());
                }
                else if ((int)selected >= ShellAPI.CMD_FIRST)
                {
                    switch (selected)
                    {
                        #region Normal Views
