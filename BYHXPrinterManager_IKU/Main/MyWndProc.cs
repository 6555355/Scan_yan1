...
//		public static bool bNcLButtonDown = false;
		...
		public static Image imgFill;//不透明背景.jpg");//pic_titbg02.gif");
		...
		public static bool LoadRenderResource(string skinPath)
		{
			try
			{
				imgBtnRes = Image.FromFile(Application.StartupPath + "\\Skins\\YouKuiKuSkinsDefault\\topbutton2.png");
				imgBtnMin = Image.FromFile(Application.StartupPath + "\\Skins\\YouKuiKuSkinsDefault\\topbutton4.png");
				imgBtnMax = Image.FromFile(Application.StartupPath + "\\Skins\\YouKuiKuSkinsDefault\\topbutton3.png");
				imgBtnClose = Image.FromFile(Application.StartupPath + "\\Skins\\YouKuiKuSkinsDefault\\topbutton1.png");
//				imgBtnCustom = Image.FromFile(Application.StartupPath + "\\Skins\\newBlue\\tab_scrollbutton.png");
				imgFill = Image.FromFile(Application.StartupPath + "\\Skins\\YouKuiKuSkinsDefault\\frametop.bmp");//不透明背景.jpg");//pic_titbg02.gif");

				imgTools = Image.FromFile(Application.StartupPath + "\\Skins\\YouKuiKuSkinsDefault\\未标题-1.png");
				imgFrameLeft = Image.FromFile(Application.StartupPath + "\\Skins\\YouKuiKuSkinsDefault\\frameVL.png");
				imgFrameRight = Image.FromFile(Application.StartupPath + "\\Skins\\YouKuiKuSkinsDefault\\frameVR.png");
				imgFrameBottom = Image.FromFile(Application.StartupPath + "\\Skins\\YouKuiKuSkinsDefault\\frameH.png");
...
//						DrawMyCaptionButton(m,mform,ButtonState.Pushed,imgBtnClose,m_rectClo);
        ...
        {
            int imargin = 2;// 按钮间距
            m_rect.X = (int)(mform.Width - SizeFrame.Width - 3 * imargin
                - CaptionButtonSize.Width -(imgBtnClose.Width + imgBtnMax.Width + imgBtnMin.Width)/4);//4 * szCaptionButton.Width);
            m_rect.Y = (int)(SizeFrame.Height + Border3DSize.Height);//6
            m_rect.Width = (int)(CaptionButtonSize.Width);
            m_rect.Height = (int)(CaptionButtonSize.Height);//20;

            m_rectMin = new Rectangle(m_rect.Right + imargin, m_rect.Y, imgBtnMin.Width/4, m_rect.Height);
            m_rectMax = new Rectangle(m_rectMin.Right + imargin, m_rect.Y, imgBtnMax.Width/4, m_rect.Height);
            m_rectClo = new Rectangle(m_rectMax.Right + imargin, m_rect.Y, imgBtnClose.Width/4, m_rect.Height);
        }
		...
		private static void DrawNormalCaption(Message m,bool isMainForm,Form mform)
		{
			...
			//画标题栏区域背景
			gs.DrawImage(imgFill,rcCaption);//显示背景图片
...
//				DrawCustomButton(gs,ButtonState.Normal,imgBtnCustom,m_rect);
		...
		{
			RectangleF rcFrameLeft = new RectangleF(0,0,xSizeFrame,mform.Height);
			RectangleF rcFrameRight = new RectangleF(mform.Width - xSizeFrame,0,xSizeFrame,mform.Height);
			...
			gs.DrawPath(SystemPens.WindowFrame, RoundRectRgn(new Rectangle(mform.Location,new Size((int)(mform.Width-SystemPens.WindowFrame.Width),(int)(mform.Height -SystemPens.WindowFrame.Width))),10));
		}
		...
		private static void DrawCustomButton(Graphics grfx, ButtonState bState,Image img,RectangleF rect)
		{
			if (img != null)
			{	
				SizeF ImageSize = new SizeF(img.Width/4,img.Height);
				...
				if (bState == ButtonState.Pushed)
				{
					...
					rectSrc = new RectangleF(new PointF(ImageSize.Width,0), ImageSize);
				else
					rectSrc = new RectangleF(new PointF(ImageSize.Width * 2, 0),ImageSize);
			...
			{
				Point loc = new Point((int)rect.Location.X,(int)rect.Location.Y);
