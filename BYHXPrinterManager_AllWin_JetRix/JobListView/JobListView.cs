...
#if ENABLECLIPTIP		
		private EditJobForm_AllWin_JetRix m_editJobForm =null;
#endif
...
//			m_StartTime = DateTime.Now;
		...
		public void OpenJob()
		{
		    if (PubFunc.IsAwBMode(m_iPrinterChange.GetAllParam()))
		    {
		...
		public void EditJob()
		{
			...
			if ((UIViewMode)m_iPrinterChange.GetAllParam().Preference.ViewModeIndex != UIViewMode.NotifyIcon)
			{
                ...
                m_editJobForm = new EditJobForm_AllWin_JetRix();
	    ...
	    private void PrintJob(UIJob job)
		{
			if(job.Status != JobStatus.Idle && job.Status != JobStatus.Printed)
				return;
            if (PubFunc.IsAwBMode(m_iPrinterChange.GetAllParam()) && !job.sJobSetting.bIsDouble4CJob)
            {
		...
		private void UpdateButtonsStates()
		{
            ...
            menuItemLLPrint.Enabled = m_MenuItemPrint.Enabled = jobOperate.CanPrintJob && m_IsFATAL;
		...
		private void m_ContextMenuJobList_Popup(object sender, System.EventArgs e)
		{
			...
			m_MenuItemPrint.Enabled = jobOperate.CanPrintJob && !BYHXSoftLock.m_DongleKeyAlarm.IsILLEGALDOG && m_IsFATAL;
